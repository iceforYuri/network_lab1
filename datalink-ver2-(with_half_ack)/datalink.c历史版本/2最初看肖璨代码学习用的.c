...
static seq_nr next_frame_to_send; // 发送方下一个要发送的帧序号
static seq_nr ack_expected;       // 发送方下一个要确认的帧序号
static seq_nr frame_expected;     // 接收方下一个要接收的帧序号
static seq_nr too_far = NR_BUFS;                // 接收方下一个要接收的帧序号
...
static int nbuffered;  // 发送方缓冲区中已存放的帧数
    ...
    for (;;)
    {
        ...
        case FRAME_RECEIVED:
            ...
            if (f.kind == FRAME_DATA)
            {
                ...
                if (between(frame_expected, f.seq, too_far)) // 序号在窗口内
                {
                    if (!arrived[f.seq % NR_BUFS]) // 窗口未满时
                    {
                        ...
                        while (arrived[frame_expected % NR_BUFS])
                        {
                            ...
                            inc(frame_expected);
                            inc(too_far);
                            start_ack_timer(ACK_TIMER);//MYM：不理解有什么用，课本上也是这么写的
                        }
                    }
                    else
                    {
                        // 这里拆分了逻辑，在收到过ACK后直接重置时钟
                        // MYM:这个分支意义在于调试 指导书：在先前启动的定时器未超时之前重新执行 start_ack_timer()调用，定时器将依然按照先前的时间设置产生超时事件 ACK_TIMEOUT
                ...
                {
                    //MYM：这个分支的意义仅仅是调试时提示落在落在了窗口外，start_ack_timer()如果按照指导书所说的则无意义
            ...
            if (f.kind == FRAME_NAK)
            {
                ...
                // 这里又进行判断重传帧是否在当前窗口 MYM：课本也有这个判断，不理解为什么
